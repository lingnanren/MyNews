from __future__ import annotations

import unittest

from modules.quality_reviewer import review_email_quality


class QualityReviewerTest(unittest.TestCase):
    def test_rejects_english_title_and_word_commas(self) -> None:
        result = review_email_quality(
            {
                "international": [
                    {
                        "title": "TrumpChinaTrade",
                        "summary": "Trump，says，China，trade，talks，continue。",
                        "source": "BBC",
                        "category": "international",
                        "is_headline": False,
                        "score": 0,
                    }
                ]
            },
            {"gregorian": "5月18日 星期一", "lunar": "四月廿二乙巳年", "ganzhi": "辛巳月 戊申日"},
        )
        self.assertFalse(result.passed)

    def test_accepts_chinese_quality_content(self) -> None:
        result = review_email_quality(
            {
                "domestic": [
                    {
                        "title": "国务院政策",
                        "summary": "国务院发布重要政策，明确下一阶段重点工作安排，涉及民生保障、产业发展和公共服务优化，相关部门将按部署推进落实，后续执行情况受到各方持续关注。",
                        "source": "中国政府网",
                        "category": "domestic",
                        "is_headline": True, "score": 100,
                    }
                ]
            },
            {"gregorian": "5月18日 星期一", "lunar": "四月廿二乙巳年", "ganzhi": "辛巳月 戊申日"},
        )
        self.assertTrue(result.passed)

    def test_rejects_missing_required_category(self) -> None:
        result = review_email_quality(
            {},
            {"gregorian": "5月18日 星期一", "lunar": "四月廿二乙巳年", "ganzhi": "辛巳月 戊申日"},
            expected_min_counts={"domestic": 1},
        )
        self.assertFalse(result.passed)


if __name__ == "__main__":
    unittest.main()
